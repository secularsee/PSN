...
def get_stable_neuron_masks(node):
    # node.lower, node.upper 是该节点 neuron 的上下界张量
    lower, upper = node.lower, node.upper
    stable_pos = (lower >= 0)
    stable_neg = (upper <= 0)
    return stable_pos, stable_neg

def get_layer_weight(node):
    # 根据 node 找到对应的权重层，这需要你结合模型结构
    # 假设 node._module 指向对应的层，且该层有 .weight
    return node._module.weight.data

def compute_stable_A(A_unstable, weight, stable_pos_mask, stable_neg_mask):
    # A_unstable 是不稳定 neuron 的 A，shape [batch, unstable_num, input_dim]
    # weight shape: [layer_out_dim, layer_in_dim]
    # 这里要根据 stable neuron 的索引，用 weight 和上一层 A 计算稳定 neuron的A
    
    # 伪代码
    A_stable_pos = torch.matmul(上一层的A, weight[stable_pos_mask, :].T)  # 恒激活 neuron 的 A
    A_stable_neg = torch.zeros_like(...)  # 恒关闭 neuron 的 A 为0
...
def combine_A(A_unstable, A_stable, unstable_idx, stable_pos_mask, stable_neg_mask):
    # 把不稳定和稳定 neuron 的 A 按照 neuron 索引顺序合并
    return A_full
...
def backward_general(
    ...
    C, batch_size, output_dim, output_shape = self._preprocess_C(C, bound_node)    #output_dim 是“你要对多少个输出元素求 bound”；
                                                                                    #output_shape 是“这些输出元素在原网络中是什么结构（比如 10 维向量 or 3×32×32 的图）
    #print("Before stopping")
    #breakpoint()  # 代码将在这一行停止，进入调试模式
    #print("After stopping")
    if initial_As is None:
        #print("start_backpropagation_at_node:")
        #print(start_backpropagation_at_node)
    ...
    queue = deque([start_backpropagation_at_node])     # 取出当前要反向处理的结点 l
    while len(queue) > 0:
        l = queue.popleft()  # backward from l              popleft()（队首弹出）

        if l.name in self.root_names:                       # 如果 l 已经是“根结点”(网络输入)，就不用再往前走
            continue
        #print("l.name:")
        #print(l)
        ...
        for l_pre in l.inputs:
            degree_out[l_pre.name] -= 1                      # 说明 l_pre 少了一个还没处理完的后继
            if degree_out[l_pre.name] == 0:         
                queue.append(l_pre)                           # 所有后继都处理完了，l_pre 可以进队列
        ...
        if l.lA is not None or l.uA is not None:
            ...
            A, lower_b, upper_b = l.bound_backward(                                    #爆显存
                lA, uA, *l.inputs,
            ...
            if self.return_A and self.needed_A_dict and bound_node.name in self.needed_A_dict:
                ...
                if len(self.needed_A_dict[bound_node.name]) == 0 or l.name in self.needed_A_dict[bound_node.name]:
                    ...
                    # 找出非根节点
                    non_root_inputs = [inp for inp in l.inputs if inp not in self.roots()]

                    # 保存矩阵列表和建立映射
                    lA_list = []
                    uA_list = []
                    input_to_idx = {}  # key: 输入节点对象或 name, value: 在列表中的索引

                    for idx, inp in enumerate(non_root_inputs):
                        if idx < len(A):
                            # 处理 lA
                            lmat = A[idx][0]
                            if lmat is not None:
                                lmat = lmat if isinstance(lmat, Patches) else lmat.transpose(0, 1).detach()
                            lA_list.append(lmat)

                            # 处理 uA
                            umat = A[idx][1]
                            if umat is not None:
                                umat = umat if isinstance(umat, Patches) else umat.transpose(0, 1).detach()
                            uA_list.append(umat)

                            # 映射
                            input_to_idx[inp.name] = idx  # 可以用 inp.name 做 key
                        else:
                            lA_list.append(None)
                            uA_list.append(None)
    ...
    if self.return_A and self.needed_A_dict and bound_node.name in self.needed_A_dict:
        ...
        save_root_A(
            ...
            self.needed_A_dict[bound_node.name],                        #在这里已经将need_A_dict换为其对应的键值
...
def get_allC_by_chunks(self, batch_size, dim_output, device, chunks=2):
    """
    将单位矩阵 eye(dim_output) 分块，避免内存爆炸。
    返回 C_chunks，每个是 [batch_size, chunk_size, dim_output]
    """
    chunk_size = dim_output // chunks
    C_chunks = []
    for i in range(chunks):
        start = i * chunk_size
        end = dim_output if i == chunks - 1 else (i + 1) * chunk_size
        size = end - start
        C = torch.zeros(batch_size, size, dim_output, device=device)
        print("C (原始):", C.shape)  # 原始未分块的C
        for j in range(size):
            C[:, j, start + j] = 1.0
        C_chunks.append(C)
...
def chunk_indices(self, num_neurons, n_chunks):
    chunk_size = num_neurons // n_chunks
    remainder = num_neurons % n_chunks

    idx_chunks = []
    start = 0
    for i in range(n_chunks):
        end = start + chunk_size + (1 if i < remainder else 0)
        idx_chunks.append(torch.arange(start, end))
        start = end
    return idx_chunks

def merge_chunk_A(self, accum_A, chunk_A, bound_node_name, start_idx, end_idx, dim_output):
    """
    将 chunk_A（来自一次 chunk 的 backward_general 返回）合并到 accum_A（累积字典）。
    - accum_A, chunk_A 的格式与 backward_general 最终保存 self.A_dict 的格式相同：
      { bound_node_name: { layer_name: { 'lA': tensor or Patches, 'uA': ..., 'lbias': ..., 'ubias': ..., 'unstable_idx': ... } } }
    - start_idx/end_idx: 本 chunk 对应到完整输出的列区间（python 左闭右开）
    - dim_output: 总输出维度
    """
    if chunk_A is None:
        return accum_A

    # chunk_A 可能只包含 bound_node_name 键
    if bound_node_name not in chunk_A:
        return accum_A

    chunk_entry = chunk_A[bound_node_name]  # dict: layer_name -> data
    if bound_node_name not in accum_A:
        accum_A[bound_node_name] = {}

    for layer_name, data in chunk_entry.items():
        # data: {"lA": tensor_or_Patches, "uA": ..., "lbias": ..., "ubias": ..., "unstable_idx": ...}
        if layer_name not in accum_A[bound_node_name]:
            # 初始化一个空占位：注意我们保留 Patches 等原样，不做合并
            accum_A[bound_node_name][layer_name] = {
                "lA": None,
                "uA": None,
                "lbias": None,
                "ubias": None,
        ...
        dest = accum_A[bound_node_name][layer_name]
        ...
        for key in ("lA", "uA"):
            ...
            if block is None:
                continue
            # 如果是 Patches（特殊对象），直接保存/覆盖（这里不做拼接）
            from auto_LiRPA.operators import Patches  # 如果 Patches 在别处定义，请按实际导入
            if isinstance(block, Patches):
                # 若没有存过，则直接写入；否则覆盖或根据需求合并（此处直接覆盖）
                dest[key] = block
                continue

            # block 是 Tensor。注意 backward_general 中保存的是 transposed 版，
            # 我们需要对齐列（输出维）进行合并。
            # 假设 block.shape = (batch, out_dim_chunk, ...)
            t = block.detach()
            # 把它展平为 [batch, out_dim_chunk * rest]，但我们只关心输出维度列位置。
            # 简化处理：将所有非 batch / output_dim 的维度合并到列后：
            batch = t.shape[0]
            # 先把 output_dim_chunk 放到第二维（应该已是），然后 collapse后面的维度：
            t_flat = t.reshape(batch, -1)

            out_chunk_dim = end_idx - start_idx
            # 检查 t_flat 的第二维是否等于 out_chunk_dim 或 out_chunk_dim * k（k表示channel flatten）
            # 我们假设这第二维“列数”与输出维的扁平化大小一致，或可以按列索引
            if t_flat.shape[1] < out_chunk_dim:
                # 形状不合预期，退回不合并（或抛错）
                raise RuntimeError(f"Unexpected A block shape {t.shape} for chunk size {out_chunk_dim}")

            # 准备目标矩阵 dest[key]：若为空则创建全 0 的展平矩阵大小 (batch, dim_output * factor)
            if dest[key] is None:
                # 目标列数 = dim_output * (t_flat.shape[1] // out_chunk_dim)
                factor = t_flat.shape[1] // out_chunk_dim
                total_cols = dim_output * factor
                dest_flat = torch.zeros(batch, total_cols, device=t.device, dtype=t.dtype)
            else:
                dest_flat = dest[key].detach().reshape(batch, -1)

            # 计算在 dest_flat 中填充的列区间
            factor = t_flat.shape[1] // out_chunk_dim
            start_col = start_idx * factor
            end_col = end_idx * factor

            # 把 t_flat 的前 out_chunk_dim*factor 列放到目标相应位置
            dest_flat[:, start_col:end_col] = t_flat[:, : (out_chunk_dim * factor)]

            # 还原回原形状（我们不知道原 shape，所以保留展平形状）
            dest[key] = dest_flat.reshape(dest_flat.shape)

        # lbias / ubias 是 bias（通常是 [batch, chunk]），我们按列合并
        for bias_key in ("lbias", "ubias"):
            b = data.get(bias_key, None)
            if b is None:
                continue
            tb = b.detach()
            batch = tb.shape[0]
            # tb 可能为 None 或 shape (out_chunk, batch) 或 (batch, out_chunk) 视情况而定（check）
            # 在 backward_general 中，lbias/ubias 被存为 transpose(0,1) 形式，可能是 (out_chunk, batch)
            # 我们做通用处理：尽量把它变成 (batch, out_chunk)
            if tb.ndim == 2 and tb.shape[0] == batch and tb.shape[1] == (end_idx - start_idx):
                tb_flat = tb
            else:
                # 尝试转置
                tb_flat = tb.transpose(0, 1)

            # 初始化目标
            if dest[bias_key] is None:
                dest[bias_key] = torch.zeros(batch, dim_output, device=tb_flat.device, dtype=tb_flat.dtype)

            dest[bias_key][:, start_idx:end_idx] = tb_flat[:, : (end_idx - start_idx)]

        # unstable_idx 这个字段可以记录对应的 chunk 索引（我们把全局索引写回）
        dest["unstable_idx"] = data.get("unstable_idx", None)
...
def save_root_A_chunk(node, A_record, A_dict, roots, needed_A_dict, lb, ub, 
                      unstable_idx, start_idx=None, end_idx=None):

    root_A_record = {}
    for root in roots:
        if root.lA is None and root.uA is None:
            continue
        if root.name not in needed_A_dict:
            continue

        if root.lA is not None:
            if isinstance(root.lA, Patches):
                _lA = root.lA
            else:
                # 先切片，再transpose
                _lA = root.lA[:, start_idx:end_idx].transpose(0, 1).detach()
                if _lA.ndim > 2:
                    batch_size = _lA.shape[0]
                    dim_feat = int(torch.prod(torch.tensor(_lA.shape[1:])))
                    _lA = _lA.reshape(batch_size, dim_feat)
        else:
            _lA = None

        if root.uA is not None:
            if isinstance(root.uA, Patches):
                _uA = root.uA
            else:
                _uA = root.uA[:, start_idx:end_idx].transpose(0, 1).detach()
                if _uA.ndim > 2:
                    batch_size = _uA.shape[0]
                    dim_feat = int(torch.prod(torch.tensor(_uA.shape[1:])))
                    _uA = _uA.reshape(batch_size, dim_feat)
        else:
            _uA = None

        lb_ = lb - root.lb if root.lb is not None else lb
        ub_ = ub - root.ub if root.ub is not None else ub

        dim_output = int(math.prod(root.output_shape[1:]))

        if root.name not in A_dict:
            batch_size = _lA.shape[0] if _lA is not None else (_uA.shape[0] if _uA is not None else 1)
            full_lA = torch.zeros(batch_size, dim_output, device=_lA.device) if _lA is not None else None
            full_uA = torch.zeros(batch_size, dim_output, device=_uA.device) if _uA is not None else None
            A_dict[root.name] = {
                "lA": full_lA,
                "uA": full_uA,
                #"lbias": None,
        ...
        if _lA is not None:
            assert _lA.shape[1] == (end_idx - start_idx), \
                f"_lA columns {_lA.shape[1]} != slice size {end_idx - start_idx}"
            A_dict[root.name]["lA"][:, start_idx:end_idx] = _lA

        if _uA is not None:
            assert _uA.shape[1] == (end_idx - start_idx), \
                f"_uA columns {_uA.shape[1]} != slice size {end_idx - start_idx}"
            A_dict[root.name]["uA"][:, start_idx:end_idx] = _uA

        A_dict[root.name]["lbias"] = lb_.detach() if lb_.ndim > 1 else None
        A_dict[root.name]["ubias"] = ub_.detach() if ub_.ndim > 1 else None

        root_A_record[root.name] = A_dict[root.name]

    root_A_record.update(A_record)
    A_dict[node.name] = root_A_record



def save_root_A(node, A_record, A_dict, roots, needed_A_dict, lb, ub, unstable_idx):
    root_A_record = A_dict.get(node.name, {})
        ...
        if roots[i].name in needed_A_dict:
            ...
            if roots[i].lA is not None:
                if isinstance(roots[i].lA, Patches):
                    ...
                    new_lA = roots[i].lA.transpose(0, 1).detach().cpu()  # 存 CPU
            else:
                new_lA = None
            ...
            if roots[i].uA is not None:
                if isinstance(roots[i].uA, Patches):
                    ...
                    new_uA = roots[i].uA.transpose(0, 1).detach().cpu()  # 存 CPU
            else:
                new_uA = None
            ...
            prev_data = root_A_record.get(roots[i].name, {})
            prev_lA = prev_data.get("lA", None)
            prev_uA = prev_data.get("uA", None)

            # 拼接 lA
            if new_lA is not None and prev_lA is not None:
                lA = torch.cat([prev_lA, new_lA], dim=1)
            elif new_lA is not None:
                lA = new_lA
            else:
                lA = prev_lA

            # 拼接 uA
            if new_uA is not None and prev_uA is not None:
                uA = torch.cat([prev_uA, new_uA], dim=1)
            elif new_uA is not None:
                uA = new_uA
            else:
                uA = prev_uA

            # 偏置也存 CPU
            #lbias = lb_.detach().cpu() if lb_.ndim > 1 else None
            #ubias = ub_.detach().cpu() if ub_.ndim > 1 else None

            root_A_record[roots[i].name] = {
                "lA": lA,
                "uA": uA,
