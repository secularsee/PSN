...
class BoundedModule(nn.Module):
    ...
    def compute_intermediate_bounds(self, node: Bound, prior_checked=False):
        #print("compute_intermediate_bounds")
        #print("node")
        #print(node)
        ...
        # For the first linear layer, IBP can give the same tightness as CROWN.
            ...
            if not skip:
                ...
                if self.return_A:
                    ...
                    num_chunks = 4 # 想分几块就设置几块
                    chunk_size = dim_output // num_chunks
                    remainder = dim_output % num_chunks
                    ...
                    # 新建空 tensor 存放最终 bounds
                    new_lower = torch.zeros(batch_size, dim_output, device=self.device)
                    new_upper = torch.zeros(batch_size, dim_output, device=self.device)

                    for start_idx, end_idx in chunk_ranges:
                        # 构造每块的 C
                        C_chunk = torch.zeros(batch_size, end_idx - start_idx, dim_output, device=self.device)
                        for b in range(batch_size):
                            C_chunk[b, :, start_idx:end_idx] = torch.eye(end_idx - start_idx, device=self.device)

                        # 调用 backward_general 计算每块的上下界
                        l_chunk, u_chunk, A_dict_chunk = self.backward_general(
                            node,
                            C_chunk,
                            unstable_idx=None,  # 如果没有特殊需要，不指定
                            apply_output_constraints_to=apply_output_constraints_to,
                            start_idx=start_idx,
                            end_idx=end_idx
                        )

                        # 合并 A_dict_chunk 到 accum_A
                        #accum_A = self.merge_chunk_A(accum_A, A_dict_chunk, node.name, start_idx, end_idx, dim_output)
                        # 将结果填入到总的 new_lower / new_upper 中
                        new_lower[:, start_idx:end_idx] = l_chunk
                        new_upper[:, start_idx:end_idx] = u_chunk
                    ...
                    # 在赋值之前 reshape
                    new_lower = new_lower.view(node.output_shape)
                    new_upper = new_upper.view(node.output_shape)
...
# ---- 新增代码开始 ----
                # 构造两个互补的C矩阵，一个覆盖稀疏不稳定神经元，一个覆盖剩余稳定神经元
                   ...
                   # 在函数顶部（分块之前）创建 accum_A_dict
                    ...
                    for start_idx, end_idx in chunk_ranges:
                        ...
                        # 调用 backward_general 计算每块的上下界
                        l_chunk, u_chunk = self.backward_general(
                            node,
                            C_chunk,
                            unstable_idx=None,  # 如果没有特殊需要，不指定
                            apply_output_constraints_to=apply_output_constraints_to,
    ...
    def save_intermediate(self, save_path=None):
        ...
        for node in self.nodes():
            if node.used and node.perturbed:
                #if not hasattr(node, 'interval'):
                if node.name not in self.A_dict.keys():
                    ...
                    # 赋值回节点
                    crown_lower = new_lower
                    crown_upper = new_upper
    ...
    from .backward_bound import (
        backward_general, get_sparse_C, concretize, get_allC_by_chunks,merge_chunk_A,
