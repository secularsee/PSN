...
class BoundedModule(nn.Module):
    ...
    def compute_intermediate_bounds(self, node: Bound, prior_checked=False):
        ...
        # For the first linear layer, IBP can give the same tightness as CROWN.
            ...
            ref_intermediate = self.get_ref_intermediate_bounds(node)   #/input-8获得interval

            sparse_C = self.get_sparse_C(node, ref_intermediate)
            newC, reduced_dim, unstable_idx, unstable_size = sparse_C
            dim_output = int(prod(node.output_shape[1:]))                   #dim_output 是输出的维度大小，比如网络输出是一个长度为 dim_output 的向量
            #allC = torch.eye(dim_output, device=self.device).expand(
                                    #self.batch_size, dim_output, dim_output)
            
            # 将输出维度一分为二，分块大小
            split_idx = dim_output // 2
            chunk_starts = [0, split_idx]
            chunk_ends = [split_idx, dim_output]
                ...
                if self.return_A:
                   ...
                   # else:
                    ...
                    #node.lower, node.upper = self.backward_general(
                        ...
                        #apply_output_constraints_to=apply_output_constraints_to)
                    #print("BBBB")
                    #node.lower, node.upper = self.backward_general(                                 #爆显存
                        #node, allC,
                        #apply_output_constraints_to=apply_output_constraints_to)
                    
# ---- 新增代码开始 ----
                # 构造两个互补的C矩阵，一个覆盖稀疏不稳定神经元，一个覆盖剩余稳定神经元
                    print("XXXX666")
                    ...
                    # 创建全零矩阵
                    C_sparse = torch.zeros(batch_size, dim_output, dim_output, device=device)
                    C_stable = torch.zeros(batch_size, dim_output, dim_output, device=device)

                    # 对稀疏神经元对应的维度，把newC的对应行拷贝到C_sparse
                    if unstable_idx is not None and len(unstable_idx) > 0:
                        # unstable_idx是不稳定神经元索引列表（1D tensor）
                        C_sparse[:, unstable_idx, unstable_idx] = 1.0  # 单位矩阵分块，或直接newC取对应行列（你实际按需替换）

                    # 其余索引为稳定神经元，放入C_stable
                    stable_idx = [i for i in range(dim_output) if (unstable_idx is None or i not in unstable_idx)]
                    if len(stable_idx) > 0:
                        C_stable[:, stable_idx, stable_idx] = 1.0

                    # 调用backward_general分别计算
                    l_sparse, u_sparse = self.backward_general(
                        node,
                        C_sparse,
                        unstable_idx=unstable_idx,
                        apply_output_constraints_to=apply_output_constraints_to
                    )
                    l_stable, u_stable = self.backward_general(
                        node,
                        C_stable,
                        unstable_idx=None,  # 对稳定神经元无不稳定索引
                        apply_output_constraints_to=apply_output_constraints_to
                    )
                   ...
                   # print("After stopping")
                    ...
                    # 拼接两个结果
                    print("l_sparse.shape", l_sparse.shape)
                    print("l_stable.shape", l_stable.shape)
                    # 初始化输出 Tensor
                    # 初始化完整维度的新 Tensor
                    new_lower = torch.zeros_like(l_sparse)
                    new_upper = torch.zeros_like(u_sparse)

                    # 注意：unstable_idx / stable_idx 是 1D list 或 tensor，需要变成 long tensor 索引
                    unstable_idx_tensor = torch.tensor(unstable_idx, device=self.device) if unstable_idx is not None else torch.tensor([], dtype=torch.long, device=self.device)
                    stable_idx_tensor = torch.tensor(stable_idx, device=self.device) if len(stable_idx) > 0 else torch.tensor([], dtype=torch.long, device=self.device)

                    # 更新稀疏神经元的位置
                    if unstable_idx_tensor.numel() > 0:
                        new_lower[:, :, :,].view(batch_size, -1)[:, unstable_idx_tensor] = l_sparse.view(batch_size, -1)[:, unstable_idx_tensor]
                        new_upper[:, :, :,].view(batch_size, -1)[:, unstable_idx_tensor] = u_sparse.view(batch_size, -1)[:, unstable_idx_tensor]

                    # 更新稳定神经元的位置
                    if stable_idx_tensor.numel() > 0:
                        new_lower[:, :, :,].view(batch_size, -1)[:, stable_idx_tensor] = l_stable.view(batch_size, -1)[:, stable_idx_tensor]
                        new_upper[:, :, :,].view(batch_size, -1)[:, stable_idx_tensor] = u_stable.view(batch_size, -1)[:, stable_idx_tensor]
    ...
    def compute_bounds(
        ...
        if C is None and target_label is not None:                                                  #C不为空、标签不为空，则计算单一维度。
            final = (self.final_node() if final_node_name is None else self[final_node_name])
            if final.output_shape is None:
                raise ValueError(f'C is missing and node {final} has no output shape')

            dim_output = int(prod(final.output_shape[1:]))

            # target_label 是 Tensor，形状为 [batch_size]
            if not isinstance(target_label, torch.Tensor):
                target_label = torch.tensor(target_label, device=self.device)

            # 构造 shape: [batch_size, 1, dim_output]
            C = torch.zeros(self.batch_size, 1, dim_output, device=self.device)
            C[torch.arange(self.batch_size), 0, target_label] = 1.0
    ...
    def save_intermediate(self, save_path=None):
        ...
        for node in self.nodes():
            if node.used and node.perturbed:
                if not hasattr(node, 'interval'):
                    ibp_lower, ibp_upper = self.IBP_general(
                        node, delete_bounds_after_use=True)

                    dim_output = int(prod(node.output_shape[1:]))

                    crown_lower = torch.zeros(
                        self.batch_size, dim_output, device=self.device)
                    crown_upper = torch.zeros(
                        self.batch_size, dim_output, device=self.device)

                    # 分块数量
                    chunks = 4
                    split_size = dim_output // chunks
                    chunk_starts = [i * split_size for i in range(chunks)]
                    chunk_ends = chunk_starts[1:] + [dim_output]

                    for start, end in zip(chunk_starts, chunk_ends):
                        chunk_size = end - start
                        C_chunk = torch.eye(chunk_size, dim_output, device=self.device)[None, :, :]
                        C_chunk = C_chunk.expand(self.batch_size, chunk_size, dim_output)

                        l_chunk, u_chunk = self.backward_general(node, C=C_chunk)

                        crown_lower[:, start:end] = l_chunk
                        crown_upper[:, start:end] = u_chunk

                    #save_dict[node.name] = (
                    #    torch.max(crown_lower, ibp_lower),
                    #    torch.min(crown_upper, ibp_upper)
                    #)
                    reshaped_shape = node.output_shape  # [B, C, H, W]
                    save_dict[node.name] = (
                        torch.max(crown_lower, ibp_lower.view(self.batch_size, -1)).view(*reshaped_shape),
                        torch.min(crown_upper, ibp_upper.view(self.batch_size, -1)).view(*reshaped_shape)
                    )
                else:
    ...
    def _compute_bounds_main(self, C=None, method='backward', IBP=False,
        ...
        if method == 'backward':
            ...
            final.interval = (final.lower, final.upper)               #新增输出上下界
            return ret
    ...
    from .backward_bound import (
        backward_general, get_sparse_C, concretize, get_allC_by_chunks,
