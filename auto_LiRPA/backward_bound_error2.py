    ...
    while len(queue) > 0:
        ...
        if l.lA is not None or l.uA is not None:
            ...
            if self.return_A and self.needed_A_dict and bound_node.name in self.needed_A_dict:
                ...
                #print(self.needed_A_dict[bound_node.name])
                if (len(self.needed_A_dict[bound_node.name]) == 0 or l.name in self.needed_A_dict[bound_node.name]) and start_idx is not None:
                ...
                elif (len(self.needed_A_dict[bound_node.name]) == 0 or l.name in self.needed_A_dict[bound_node.name]) and start_idx is None:
                    # A could be either patches (in this case we cannot transpose so directly return)
                    # or matrix (in this case we transpose)
                    A_record.update({
                        l.name: {
