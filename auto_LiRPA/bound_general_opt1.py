...
# ---- 新增代码开始 ----
                # 构造两个互补的C矩阵，一个覆盖稀疏不稳定神经元，一个覆盖剩余稳定神经元
                    ...
                    # 填入稀疏部分（前半部分通道）
                    new_lower[:, :split_idx] = l_sparse[:, :split_idx]
                    new_upper[:, :split_idx] = u_sparse[:, :split_idx]

                    # 填入非稀疏部分（后半部分通道）
                    new_lower[:, split_idx:] = l_stable[:, split_idx:]
                    new_upper[:, split_idx:] = u_stable[:, split_idx:]
