        ...
        if l.lA is not None or l.uA is not None:
            ...
            if self.return_A and self.needed_A_dict and bound_node.name in self.needed_A_dict:
                ...
                if len(self.needed_A_dict[bound_node.name]) == 0 or l.name in self.needed_A_dict[bound_node.name]:
                    ...
                    # 预先初始化（只在第一次遇到这个 l.name 时做一次）
                    if l.name not in A_record:
                        dim_output = int(prod(l.output_shape[1:]))                   #dim_output 是输出的维度大小，比如网络输出是一个长度为 dim_output 的向量
                        A_record[l.name] = {
                            "lA": torch.zeros(dim_output, batch_size, device=self.device),
                            "uA": torch.zeros(dim_output, batch_size, device=self.device),
                            # 如果有 bias，也一并初始化
                            "lbias": torch.zeros(dim_output, device=self.device),
                            "ubias": torch.zeros(dim_output, device=self.device)
                        }

                    # 每次分块 backward_general 得到当前块的 A
                    lA_chunk = A[0][0].transpose(0, 1).detach()   # 形状: (chunk_size, batch_size)
                    uA_chunk = A[0][1].transpose(0, 1).detach()

                    # 把当前块结果写回到 A_record 对应位置
                    A_record[l.name]["lA"][start_idx:end_idx] = lA_chunk
                    A_record[l.name]["uA"][start_idx:end_idx] = uA_chunk
                    lbias_chunk = lb.transpose(0, 1).detach() if lb.ndim > 1 else None,
                    ubias_chunk = ub.transpose(0, 1).detach() if ub.ndim > 1 else None,
                    # 如果 bias 也需要记录，同样分块填充
                    if "lbias" in A_record[l.name]:
                        A_record[l.name]["lbias"][start_idx:end_idx] = lbias_chunk
                        A_record[l.name]["ubias"][start_idx:end_idx] = ubias_chunk
            ...
            for i, l_pre in enumerate(l.inputs):
                ...
                #breakpoint()  # 代码将在这一行停止，进入调试模式
                #print("After stopping") 
                add_bound(l, l_pre, lA=A[i][0], uA=A[i][1],start_idx=start_idx, end_idx=end_idx)           #分发新系数：A 的每一行对应一个输入节点 l_pre。

                                                                    #add_bound() 会把 A[i][0/1] 写入 l_pre.lA/uA。

                                                                    #当所有后继都处理完，degree_out[l_pre] 归零，它就进队列，开始对它的前驱再求 bound_backward()。
...
def add_bound(node, node_pre, lA=None, uA=None, start_idx=None, end_idx=None):
    print(f"add_bound called with start_idx={start_idx}, end_idx={end_idx}")
    ...
    def update_A(existing_A, new_A, start_idx, end_idx):
        if start_idx is None or end_idx is None:
            return addA(existing_A, new_A)
        else:
            existing_A_flat = existing_A.view(existing_A.shape[0], -1)
            new_A_flat = new_A.view(new_A.shape[0], -1)
            existing_A_flat[start_idx:end_idx] += new_A_flat
            existing_A.copy_(existing_A_flat.view_as(existing_A))
            return existing_A
    ...
    if lA is not None:
        if node_pre.lA is None:
            shape = (dim_output,) + lA.shape[1:]
            node_pre.lA = torch.zeros(shape, device=lA.device) # 保持形状
            if start_idx is not None:
                node_pre.lA[start_idx:end_idx] = lA
            else:
                node_pre.lA = lA
        else:
            node_pre.lA = update_A(node_pre.lA, lA, start_idx, end_idx)
        print("Before stopping")
        breakpoint()  # 代码将在这一行停止，进入调试模式
        print("After stopping")
    ...
    if uA is not None:
        if node_pre.uA is None:
            node_pre.uA = torch.zeros_like(uA)
            if start_idx is not None:
                node_pre.uA[start_idx:end_idx] = uA
            else:
                node_pre.uA = uA
        else:
            node_pre.uA = update_A(node_pre.uA, uA, start_idx, end_idx)
        ...
        if roots[i].name in needed_A_dict:
            ...
            if roots[i].lA is not None:
                if isinstance(roots[i].lA, Patches):
                    ...
                    #_lA = roots[i].lA.transpose(0, 1)                                 #保留梯度
            else:
                _lA = None
